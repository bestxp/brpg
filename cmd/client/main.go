...
func (g *Game) Draw(screen *e.Image) {
	...
	ebitenutil.DebugPrint(screen, fmt.Sprintf("TPS: %0.2f \n VS: %v", e.CurrentTPS(), e.IsVsyncEnabled()))
}
...
func init() {
	...
	levelImage, err = prepareLevelImage(engine.LoadLobby())
	if err != nil {
		log.Fatal(err)
	}
}
...
func main() {
	...
	game := &Game{Conn: c, Width: float64(config.width), Height: float64(config.height)}
	if err := e.RunGame(game); err != nil {
		log.Fatal(err)
	}
}

func prepareLevelImage(level [][]string) (*e.Image, error) {
	tileSize := 16
	width := len(level[0])
	height := len(level)
	levelImage := e.NewImage(width*tileSize*config.scale, height*tileSize*config.scale)
...
func handleKeyboard(c *websocket.Conn) {
	...
	if e.IsKeyPressed(e.KeyQ) {
		var err error
		levelImage, err = prepareLevelImage(engine.LoadLevel())
		if err != nil {
			log.Println(err)
		}
	}
